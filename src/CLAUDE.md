...
- **주요 토픽/TF**
  - 구독(Subscriber)
    - `/cone/fused/ukf` (`custom_interface::msg::TrackedConeArray`, QoS=BestEffort/Volatile, depth=10) → `cone_callback`
...
1. `/cone/fused/ukf` 수신 → 센서→`base_link` 변환 → 전처리
...
- 토픽: 구독(`/cone/fused/ukf`, `/odometry/filtered`), 퍼블리시(`/slam/pose`, `/slam/odometry`)
