...
### 1. `/cone/fused/ukf` [custom_interface/msg/TrackedConeArray] ✅ 권장
...
### 1-ALT. `/cone/fused/ukf/map` [custom_interface/msg/TrackedConeArray] (비권장/주의)
...
<node pkg="cone_stellation" exec="cone_slam_node" name="cone_slam">
  <remap from="/cone/for_sim" to="/your/cone_topic"/>
...
- Cone detections should be provided in ORIGINAL sensor frame with valid TF to `base_link` (권장: `/cone/fused/ukf`)
